<script>
  ...
  // Use $derived instead of reactive statement for Svelte 5 runes
  ...
  async function fetchData() {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5058/api/gk-transactions?${params}`,
      );
    ...
    <div class="table-container">
      <table class="data-table">
        <thead>
          <tr>
            <th
              class="frozen sortable"
              onclick={() => sortData("GK_GL_BR_TEM")}
            >
            ...
            <tr>
              ...
              <td>
                <div class="action-buttons">
                  <button
                    class="btn-edit"
                    onclick={() => editTransaction(item.GK_GL_BR_TEM)}
                    >Edit</button
                  >
                  <button
                    class="btn-delete"
                    onclick={() => deleteTransaction(item.GK_GL_BR_TEM)}
                    >Delete</button
                  >
                </div>
              </td>
