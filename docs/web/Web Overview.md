...
- outbound web requests to external services 
...
5. optionally wait synchronously for result if `justFire = false` 
...
- `Sql2Json(sqlResult2D)` to transform SQL query output into JSON string 
...
- return `Response(...)` when headers or custom status code are needed 
...
- sending multipart requests with files 
...
- Prefer template functions over manual HTML string assembly when template rendering is intended.