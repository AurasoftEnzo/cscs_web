...
| `justFire` | boolean | no | If `true`, fire asynchronously without waiting for response; if `false`, wait and return response message | [file:104]
...
If `justFire = true`, the function sends the request asynchronously and does not return a response payload [file:104].
...
- optionally needing synchronous response retrieval [file:104]
...
- Omitting required authorization headers. [file:104]
...
- Use `justFire = true` for background notifications or non-blocking integrations. [file:104]
...
- `justFire` controls sync versus async behavior and also whether the function returns response text. [file:104]