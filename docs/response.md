...
Use it when the handler must explicitly control headers, body and status code [file:104].
...
| `statusCode` | integer | yes | HTTP status code | [file:104]

## Returns
Returns a response object that can be returned from an endpoint handler [file:104].
...
- non-default HTTP status code is needed [file:104]
...
- `Cache-Control`
- custom application headers [file:104]
...
Reason:
`headers` must be a dictionary / hash-table, not a flat string [file:104].
...
- Use consistent error body structure for API endpoints. [file:104]
...
- For APIs, explicit response construction is preferable to implicit string returns. [file:104]